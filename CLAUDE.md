...
8. **Update detection (no needless clone)**: `build()` decides whether to publish via two gates — (1) local `state/*.json` match, then (2) the AUR RPC API (`aur.current_version()`, `https://aur.archlinux.org/rpc/v5/info`), which is authoritative and clone-free. A package already at the target pkgver is skipped even if its state file is missing/stale, so the AUR repo is only cloned when there's a real update. The build's decision (created vs updated) is threaded into `publish()` as `is_new`, so the publish step no longer re-derives existence over SSH.
9. **Transient AUR failures (maintenance/outage)**: All network-bound git ops (`ls-remote`/`clone`/`pull`/`push`) retry transient errors up to 3× with backoff via `aur._git_run_with_retry` (deterministic errors like `permission denied`/`not found` fail fast). `publish()` reuses the build step's RPC-authoritative created/updated decision (`is_new`) instead of re-deriving existence over SSH, and `exists()` raises `AurUnavailableError` on unreachable/AUR-down instead of guessing "new package".
